<x-filament-actions::action :action="$action" :badge="$getBadge()" :badge-color="$getBadgeColor()"
    dynamic-component="filament::button" :label="$getLabel()" :size="$getSize()" class="fi-ac-icon-btn-action"
    color="gray">
    <style>
        .folder-icon-{{$item->id}} {
            width: 100px;
        ...
        <div class="flex flex-col items-center justify-center my-2">
            ...
            <div class="flex justify-start mt-1">
                <p class="text-gray-600 dark:text-gray-300 text-sm truncate ...">
                    User: {{ $item->usersUser?->name ?? 'Public'}}
                </p>
            </div>
